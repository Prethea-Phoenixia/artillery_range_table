...
def dekker(f: Callable[[float], float], x_0: float, x_1: float, tol: float, max_it: int = 33) -> tuple[float, float]:
    ...
    and mid-point estimate. If this results in a sign change, the contra-point is updated
    ...
    for i in range(max_it):
        ...
        if fa_j * fb_k < 0:
            # if the contra-point is of different sign than current estimate
            (a_k, fa_k) = (a_j, fa_j)  # new contra-point is still the same
        else:  # otherwise, new contra-point should use the current est.
            (a_k, fa_k) = (b_j, fb_j)
        ...
        # the entire loop has run without break or return.
